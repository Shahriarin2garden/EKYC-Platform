...
const PdfButton: React.FC<PdfButtonProps> = ({ kycId, onSuccess, onError }) => {
  ...
  if (pdfGenerated) {
    return (
      <button
        ...
        disabled={loading}
        className={`p-2 text-brand-accent hover:bg-brand-accent/10 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer ${
      ...
      >
        {loading ? (
          <svg className="w-5 h-5 animate-spin pointer-events-none" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ) : (
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`p-2 text-brand-white/70 hover:text-brand-white hover:bg-brand-white/10 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer ${
        loading ? 'animate-pulse' : ''
      }`}
